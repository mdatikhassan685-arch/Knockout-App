    ...
    try {
        ...
        if (type === 'join_daily_match') {
            ...
            try {
                ...
                // Insert All Players with GAME LEVEL
                const tName = (match.match_type !== 'Solo' && team_name) ? team_name : 'Solo';

                for (let p of players) {
                    // Check database if column exists logic assumed. 
                    // Running query directly:
        ...
        if (type === 'get_daily_room') {
            ...
            if(chk.length > 0) {
                ...
                // Send room info ONLY if it exists
                if (r[0].room_id && r[0].room_id !== 'null' && r[0].room_id !== '') {
                    return res.status(200).json(r[0]);
                }
            }
            return res.json({}); // Returns empty if no room yet, handled by frontend
        }

        // --- 4. GET PARTICIPANTS (Fetching Level) ---
        if (type === 'get_daily_participants') {
            const [rows] = await db.execute(`
                SELECT game_name, team_name, game_uid, game_level, kills, prize_won, joined_at 
                FROM match_participants 
                WHERE match_id = ? 
                ORDER BY team_name DESC, joined_at ASC
            `, [match_id]);
